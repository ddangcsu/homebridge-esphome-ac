...
module.exports = {
	init: function() {
		...
		this.devices.forEach(device => {
			...
			const addNewAccessory = entity => {
				...
				if (entity.type === 'Climate') {
					...
					if (entity.id === 3877880146) {
						// Air Conditioner Outdoor Temperature
						entity.once('state', (state) => {
							this.log.easyDebug(`${device.name} Entity State:`)
							this.log.easyDebug(state)
							this.log(`Initializing Outdoor Temperature Sensor Accessory - ${device.name}`)
							this.esphomeDevices[entity.config.uniqueId] = new TemperatureSensor(device, entity, state, this)
							client.off('newEntity', addNewAccessory);
					...
					} else if (entity.id === 2854408378 ) {
						// Indoor Humidity Sensor
						entity.once('state', (state) => {
							this.log.easyDebug(`${device.name} Entity State:`)
							this.log.easyDebug(state)
							this.log(`Initializing Indoor Humidity Sensor Accessory - ${device.name}`)
							this.esphomeDevices[entity.config.uniqueId] = new HumiditySensor(device, entity, state, this)
							client.off('newEntity', addNewAccessory);
		...
		this.accessories.forEach(accessory => {
			...
			this.log.easyDebug(`Unregistering deleted device: "${accessory.displayName}" | ID:${accessory.context.deviceId} | Host: ${accessory.context.host} `)
